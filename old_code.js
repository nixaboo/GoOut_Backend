...
async function doStuff2() {
    ...
    //here we would add to the Jobs runner
}


async function getShowAjax(showId)
{
...
async function testClipa() {
    ...
    var body = fs.readFileSync('clipa_show_page.html');
    var doc = new dom().parseFromString(body.toString())
    //var nodes = xpath.select('//div[contains(@class, "prod-details")]', doc)
    //var nodes = xpath.select('//li/[contains(text(),"תאריך")]', doc);
    // var nodes = xpath.select('//span/parent::li/span[contains(text(),"תאריך")]/parent::li//text()', doc);
    var nodes = xpath.select('//form[contains(@class, "variations_form") and contains(@class, "cart")]//*[contains(@class, "woocommerce-Price-amount")]/text()', doc);
    ...
    var showsData = allShowDivs.map(showDiv => {
        ...
        showRows.forEach(showRow => {
            var show = {
                ...
                //time : showRow.findAll('span', 'showTime')[0].ge tText(),
    ...
    showsData = await Promise.map(showsData, async function fetchShowFrame(show) {
        ...
        async function e(options, depth){
